...
export class FabricGuide extends Disposable {
  ...
  private aligningLineWidth = 1.5;
  private aligningLineColor = "#22319e";
  ...
  private objectMoving({ target }: any) {
    ...
    const add = (group: Group | Canvas | ActiveSelection) => {
      const objects = group.getObjects().filter((obj) => {
        ...
        // Exclude self and activate elements within the selection
        ...
        //The element is a group, add the elements in the group and exclude the group itself
    ...
    if (check.isActiveSelection(target)) {
      const needAddParent = new Set<Group | Canvas>();
  ...
  // When the object is rotated, some coordinates need to be ignored. For example, the horizontal auxiliary line only takes the top and bottom coordinates (refer to figma)
  private omitCoords(
  ...
  /**
   * Checks whether value1 and value2 are within the specified range for calculation of alignment lines
   */
