...
/** Generates an object representing the offset of each value in the drawing range */
...
/** Update the drawing range according to the current mouse position */
...
/** Redraw the image in the drawing board when transforming (translating, scaling) */
...
/** Update transformation parameters when zooming with mouse wheel */
function updateTransformConfigWhileScaling(ev: WheelEvent, transformConfig: TransformConfig) {
  ...
  // You cannot use deltaRatio directly. When scaleRatio is close to the maximum/minimum value, the two are not equal.
...
/** Calculate the position of the pixel corresponding to the mouse position in the image */
...
/** Calculate the new drawing range */
...
/** Calculate the new drawing range value in the x/y axis direction after scaling */
