  ...
  <div v-if="hasHeader" class="options-container">
    <slot>
      <div class="default-wrap">
        <div class="option">
          <label for="image">تصویر را انتخاب کنید</label>
          <input id="image" type="file" accept=".jpg,.png,.gif,.webp" @change="onFileChange" />
        </div>
        <div class="option">
          <span>نوع قلم</span>
          <label for="fix">تعمیر</label>
          <input id="fix" v-model="isErasing" type="radio" :value="false" />
          <label for="matting">پاک کردن</label>
          <input id="matting" v-model="isErasing" :value="true" type="radio" />
        </div>
        <div class="option">
          <label for="radius">اندازه قلم:</label>
          <input id="radius" v-model="radius" class="range-input" type="range" :max="RADIUS_SLIDER_MAX"
            :min="RADIUS_SLIDER_MIN" :step="RADIUS_SLIDER_STEP" />
          <span>{{ brushSize }}</span>
        </div>
        <div class="option">
          <label for="hardness">Brush Hardness: </label>
          <input id="hardness" v-model="hardness" class="range-input" type="range" :max="HARDNESS_SLIDER_MAX"
...
export default defineComponent({
  props: {
    // Whether there is a header toolbar in the component
    hasHeader: {
      type: Boolean,
      default: true,
    },
  ...
  setup(props, { emit }) {
    ...
    const saveBtnText = computed(() => (generating.value ? 'Saving...' : 'save'))
    ...
    const onFileChange = (ev: Event) => {
      ...
      if (files && files[0] && /.+\.(jpg|jpeg|png|gif|webp)/.test(files[0].name)) {
        ...
        alert('No image selected or the image format is incorrect(Only supports jpg, png, gif, webp), Please reselect')
      }
    }
    ...
    const initLoadImages = (source: string, result: string) => {
      nextTick(() => {
        fetch(source)
          ...
          .catch((error) => {
            console.error('Failed to get the image:', error)
          })
        ...
        watch(
          ...
          async () => {
            ...
            // TODO Draw the ImageBitmap onto the canvas
            ...
            // Traverse each pixel and draw the non-transparent pixels as a mask
            for (let i = 0; i < data.length; i += 4) {
              if (data[i + 3] > 0) {
                ...
                data[i + 3] = (DEFAULT_MASK_COLOR[3] - 0.15) * 255 // The color is lighter, so it is easier to distinguish
              }
            }
            await nextTick()
            setTimeout(async () => {
...
.board-container {
  ...
  .matting-cursor {
    /** Penetrate the brush and trigger the canvas click event */
    pointer-events: none;
    display: none;
    position: absolute;
    left: -9999px;
