<template>
  <el-dialog v-model="state.dialogVisible" :title="'برش تصویر'" width="35%" class="matting-dialog"
    :before-close="closeUpload" :close-on-click-modal="false">
    <el-row class="model-row">
      <el-col :span="2" class="model-tip">مدل</el-col>
      <el-col :span="4">
    ...
    <el-upload v-if="!state.originImage" class="upload-demo" ref="uploadRef" :on-exceed="handleExceed" drag
      action="http" :http-request="uploadHandle" :limit="1" :accept="state.fileAccept" v-loading="state.loading">
      <el-icon :size="50">
        <UploadFilled />
      </el-icon>
      <div class="el-upload__text">
        فایل‌ها را در اینجا بکشید و رها کنید یا<em> آپلود فایل را انتخاب کنید</em>
      </div>
      <template #tip>
        <div class="el-upload__tip">برای تجربه بهتر، لطفاً تصاویر را در 2 مگابایت آپلود کنید</div>
      </template>
    </el-upload>
    <div class="content">
      <div v-show="state.originImage" v-loading="!state.resultImage"
        :style="{ width: state.offsetWidth ? state.offsetWidth + 'px' : '100%' }"
        class="scan-effect transparent-background">
        <img ref="raw" :style="{ 'clip-path': 'inset(0 0 0 ' + state.percent + '%)' }" :src="state.originImage"
          alt="Raw Image" />
        <img v-show="state.resultImage" :src="state.resultImage" @mousemove="mousemove" alt="Result Image" />
        <div v-show="state.resultImage" :style="{ left: state.percent + '%' }" class="scan-line"></div>
      </div>
    </div>
    <template #footer>
      <div class="dialog-footer">
        <el-button v-show="state.originImage && state.toolModel" type="danger" @click="clear">پاک کردن</el-button>
        <el-button v-show="state.resultImage" type="primary" @click="edit">ویرایش</el-button>
        <el-button v-show="state.resultImage && state.toolModel" type="success" @click="download">دریافت</el-button>
    ...
    : 'برش تصویر کامل'
        ...
        <el-button v-show="state.resultImage && props.image" v-loading="state.loading" type="primary"
          @click="replaceImage">
          replace
        </el-button>
      </div>
...
const download = () => {
  ...
  downloadLinkFile(
    state.resultImage,
    `editor-${Date.now()}-matting-${state.filename}`
  );
};
