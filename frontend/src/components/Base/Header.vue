<template>
    <div v-if="false"
        ...
        <div class=" items-center gap-4 md:flex hidden">
            <el-tooltip class="box-item" effect="dark" v-if="user?.role" content="Exit" placement="top">
                <button @click="handleLogOut"
                    class="hover:bg-primary-b3 hover:bg-opacity-30 w-[40px] h-[40px] rounded-xl flex items-center justify-center">
                    <Icon name="log-out" />
                </button>
            </el-tooltip>
            <el-tooltip class="box-item" effect="dark" content="Announcements" placement="top">
                <button
            ...
            <button
                ...
                v-if="user?.role">
                <img :src="`/images/avatars/${user?.role}.png`" class="h-14 w-14 p-1" style="border-radius:12px" />
                <div class="flex flex-col items-start">
                    <p class="text-sm font-semibold text-gray-500">{{ user?.role == 'admin' ? 'Manager' : user?.role ==
                'designer' ?
            ...
            <Button
                ...
                v-else @click="isLoginDialogVisible = !isLoginDialogVisible">
                ورود
            </Button>
        </div>
        <button class="w-10 h-10 bg-white rounded-xl md:hidden flex items-center justify-center"
            @click="isDrawerVisible = !isDrawerVisible">
            <icon name="text-align" />
        </button>
        <el-drawer v-model="isDrawerVisible" direction="rtl" size="80%" :with-header="false" class="p-5">
            ...
            <main>
                <div class="flex items-center justify-between">
                    <template v-if="user?.role">
                        <button
                            class="hover:bg-primary-b3 hover:bg-opacity-50 p-1 rounded-xl flex items-center justify-center gap-2">
                            <img :src="`/images/avatars/${user?.role}.png`" class="h-14 w-14 p-1"
                ...
                <ul class="mt-6 grid gap-2">
                    <li v-for="(item, index) in menuOptions" :key="index" class="w-full h-full">
                        <RouterLink :to="item.route"
                            class="w-full h-full py-2 flex items-center gap-1 text-sm px-1 rounded-xl"
                            :class="item.route == route.path ? 'text-primary-b1 bg-primary-b3 bg-opacity-30' : 'text-black md:bg-opacity-15 bg-opacity-0'">
