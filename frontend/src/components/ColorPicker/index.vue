<template>
  <div class="color-picker">
    ...
    <div class="picker-presets">
      <div class="picker-presets-color" v-for="c in themeColors" :key="c" :style="{ background: c }"
        @click="selectPresetColor(c)"></div>
    </div>

    <div class="picker-gradient-presets">
      <div class="picker-gradient-col" v-for="(col, index) in presetColors" :key="index">
        <div class="picker-gradient-color" v-for="c in col" :key="c" :style="{ background: c }"
          @click="selectPresetColor(c)"></div>
      </div>
    </div>

    <div class="picker-presets">
      <div v-for="c in standardColors" :key="c" class="picker-presets-color" :style="{ background: c }"
        @click="selectPresetColor(c)"></div>
    </div>
...
// Each time you select a non-preset color, you need to add the color to the recently used list
...
// Check whether the environment supports the native color picker. If it supports, use the native color picker. Otherwise, use the custom color picker.
...
const browserEyeDropper = () => {
  ElMessage({
    type: 'success',
    message: 'Press the ESC key to close the color picker'
  })
  ...
  eyeDropper.open().then((result: { sRGBHex: string }) => {
    ...
    ElMessage({
      type: 'success',
      message: 'Close the color picker'
    })
  })
}

// Custom color picker based on Canvas
const customEyeDropper = () => {
  ...
  toCanvas(targetRef, { filter, fontEmbedCSS: '', width, height, canvasWidth: width, canvasHeight: height, pixelRatio: 1 }).then(canvasRef => {
    ...
    ElMessage({
      type: 'error',
      message: 'Color picker initialization failed'
    })
    document.body.removeChild(maskRef)
  })
}
