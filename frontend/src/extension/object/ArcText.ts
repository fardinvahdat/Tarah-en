import { IText as OriginIText, Textbox as OriginTextbox, Object as FabricObject, Control, controlsUtils, Color,
  classRegistry, Transform, TPointerEvent, util, Point, TMat2D, TFiller,
  Gradient
} from 'fabric'
...
export class ArcText extends OriginIText {
  ...
  protected _charTransformations: any = [];
  ...
  public getCharTransformations() {
    return this._charTransformations;
  }
  ...
  private _applyGradientToContext(
    ctx: CanvasRenderingContext2D,
    fill: TFiller,
    charBounds?: any,
    charTransform?: any
  ): void {
    if (fill && typeof fill === "object" && (fill as Gradient).type) {
      const gradient = fill as Gradient;
      let canvasGradient: CanvasGradient;

      if (gradient.type === "linear") {
        // Use the gradient's actual coordinates or default values
        const coords = gradient.coords || { x1: 0, y1: 0, x2: 100, y2: 100 };

        // Apply character-specific transformations if available
        let x1 = coords.x1,
          y1 = coords.y1,
          x2 = coords.x2,
          y2 = coords.y2;

        if (charBounds && charTransform) {
          // Transform coordinates based on character position and bounds
          const scaleX = charBounds.width || 1;
          const scaleY = charBounds.height || this.fontSize;

          x1 = (coords.x1 / 100) * scaleX - scaleX / 2;
          y1 = (coords.y1 / 100) * scaleY - scaleY / 2;
          x2 = (coords.x2 / 100) * scaleX - scaleX / 2;
          y2 = (coords.y2 / 100) * scaleY - scaleY / 2;
        }
        ...
        // Apply character-specific transformations for radial gradients
        let x1 = coords.x1,
          y1 = coords.y1,
          r1 = coords.r1;
        let x2 = coords.x2,
        ...
        if (charBounds) {
          const scale =
            Math.min(charBounds.width || this.fontSize, this.fontSize) / 2;
          r1 = (coords.r1 / 100) * scale;
          r2 = (coords.r2 / 100) * scale;
        }
      ...
      // Ensure all color stops are added, including first and last
      if (gradient.colorStops && gradient.colorStops.length > 0) {
        // Sort color stops by offset to ensure proper order
        const sortedStops = [...gradient.colorStops].sort(
          (a, b) => a.offset - b.offset
        );

        sortedStops.forEach((colorStop) => {
  ...
  runCharRendering(
    ...
    if (ctx) {
      ...
      // Apply gradient with character-specific bounds and transform info
      if (method === "fillText" || method === "both") {
        const charBounds = { width: this.fontSize, height: this.fontSize };
        const charTransform = { left, top, angle };
        this._applyGradientToContext(
          ctx,
          fullDecl.fill,
          charBounds,
          charTransform
    ...
    for (let i = 0; i < this.__charBounds.length; i++) {
      ...
      for (let j = 0; j < row.length; j++) {
        ...
        if (ct.char?.trim() && bounds.contour) {
          ...
          let matrix = util.multiplyTransformMatrices(
  ...
  defaultTextRender(
    ...
    if (method === "both") {
      if (decl.fill && this.paintFirst === "fill") {
        // Apply gradient for fill - no need to call _applyGradientToContext again as it's already applied in runCharRendering
      ...
      if (decl.fill && this.paintFirst === "stroke") {
        // Apply gradient for fill - no need to call _applyGradientToContext again
        ctx.fillText(_char, 0, 0);
      }
    } else {
      if (method === "fillText" && decl.fill) {
        // Apply gradient for fill - no need to call _applyGradientToContext again
        ctx.fillText(_char, 0, 0);
      }
      if (method === "strokeText" && decl.stroke && decl.strokeWidth) {
        ctx.strokeText(_char, 0, 0);
      }
    }
    return true;
