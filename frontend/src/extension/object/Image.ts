...
export class Image extends OriginImage {
  ...
  public doubleClickHandler(e: TPointerEventInfo<TPointerEvent>) {
    ...
    // Check image size before enabling cropping to prevent performance issues
    const imageSize = this.getEstimatedImageSize()
    if (imageSize > 2 * 1024 * 1024) { // 2MB threshold
      console.warn('🚨 Large image detected, triggering performance optimization')
      window.dispatchEvent(new CustomEvent('selection:emergency-optimize'))
    }
    
    this.set({__isCropping: true, clipPath: undefined})
    this.canvas.setActiveObject(this as any)
    
    // Defer render to next frame for smoother interaction
    requestAnimationFrame(() => {
      this.canvas?.requestRenderAll()
    })
  }

  // Estimate image memory footprint
  private getEstimatedImageSize(): number {
    try {
      const element = this._originalElement || this._element
      if (!element) return 0
      
      const width = (element as any).naturalWidth || (element as any).width || 0
      const height = (element as any).naturalHeight || (element as any).height || 0
  ...
  set _cropKey(key) {
    ...
    this.cropKey = key
    this.setCropCoords(this.width, this.height);
  }

  // initialization
  initEffects () {
    // Cache the original fields for recovery and drawing new images from the original images
