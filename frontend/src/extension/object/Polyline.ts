...
import type { Group, Canvas, ActiveSelection, TSVGReviver } from "fabric";
import {
...
export class Polyline extends OriginPolyline {
  ...
  public pointMoving(pointIndex: number, point: Point): Point {
    ...
    const add = (group: Group | Canvas | ActiveSelection) => {
      const objects = group.getObjects().filter((obj) => {
        ...
        // Excludes itself and the elements in the active selection
        ...
        // Elements are groups, and the elements in the group are added, while excluding the group itself
  ...
  private snap({
    ...
    /** The coordinates of the active object */
    point: Point;
