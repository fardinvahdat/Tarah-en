...
export class SVGImage extends Image {
  ...
  static async fromObject({ filters: f, resizeFilter: rf, src, crossOrigin, ...object }: any, options: { signal: AbortSignal }): Promise<Image> {
    // Preserve original dimensions and scaling information
    ...
    return Promise.all([
      ...
      // Apply stored dimensions and scaling
  ...
  toObject<T extends Omit<Partial<ImageProps> & TClassProperties<this>, keyof SerializedImageProps>, K extends keyof T = never>(propertiesToInclude?: K[] | undefined): Pick<T, K> & SerializedImageProps {
    ...
    // Ensure original dimensions and scale are preserved in serialized data
    if (this.originWidth) object.originWidth = this.originWidth
    if (this.originHeight) object.originHeight = this.originHeight
    if (this.srcWidth) object.srcWidth = this.srcWidth
    if (this.srcHeight) object.srcHeight = this.srcHeight
    if (this.originScale) object.originScale = this.originScale
    
    return object
  }
  
  // Override setSrc to maintain dimensions when changing the image
  setSrc(src: string, callback?: Function) {
    ...
    // Call original setSrc
    super.setSrc(src, () => {
      // After loading, restore original dimensions and scale
      this.set({
        width: originalWidth,
...
classRegistry.setSVGClass(SVGImage)
