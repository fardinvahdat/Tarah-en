...
<template>
  <el-dialog v-model="dialogVisible" :before-close="closeColor" class="self-background lg:!w-1/2 !w-[90%]">
    <div class="self-main">
    ...
    <div class="self-footer">
      <label class="self-lable">برای تغییر رنگ روی مربع کلیک کنید</label>
      <div class="self-color">
        <div class="color-content">
          <div v-for="item in gridColorSelf" :key="item.index" :style="{backgroundColor: item.color}"
            class="color-item">
          ...
          <el-button class="ml-5" circle @click="addColor" v-if="gridColorSelf.length < 11">
            <!-- <IconPlus /> -->
          </el-button>
          <el-button class="ml-5" circle @click="subColor" v-if="gridColorSelf.length > 2">
            <!-- <IconMinus /> -->
          </el-button>
        </div>
      </div>
      <el-button class="btn close" round @click="saveColor()">ذخیره رنگ</el-button>
    </div>
  </el-dialog>
</template>
