<template>
    <div class="sticky -top-2 h-18 p-2 flex items-center justify-between flex-row  md:bg-primary-b1 md:gap-0 gap-2 z-20">
        <div class="md:pr-2 pr-1 flex items-center md:gap-4 gap-2">
            <Button class="bg-primary-b3 px-4 py-1 h-fit md:!text-sm !text-xs text-primary-default"
                @click="isExportDialogVisible = true">Receive</Button>
            <div class="flex items-center flex-row-reverse md:gap-1">
                ...
                <el-tooltip placement="top" :hide-after="0">
                    ...
                    <button @click="changeRuler"
                        class="md:text-white text-black w-9 h-9 hover:bg-white/40 rounded-md cursor-pointer flex items-center justify-center"
                        :class="rulerEnabled ? 'ruler-enabled bg-white/40 md:text-white text-black' : 'md:text-white/60 text-black/60'">
                        <Icon name="ruler" class="md:w-6 md:h-6 w-6 h-6" />
                    </button>
                </el-tooltip>
            </div>
            <div class="md:hidden flex items-center gap-1">
                <button @click="scaleCanvas('+')" class="handler-item md:px-2 text-black">
                    <Icon name="zoom-in" class="w-6 h-6" />
                </button>
                <el-popover placement="bottom" trigger="click" width="100" popper-class="viewport-size">
                    <template #reference>
                        <span class="text text-black md:text-sm text-xs" ref="scaleRef">{{ canvasZoom }}</span>
                    </template>
                    <div class="viewport-size-preset">
                        <div class="preset-item py-2 text-left" v-for="item in canvasZoomPresets" :key="item"
                ...
                <button class="handler-item md:px-2 text-black" @click="scaleCanvas('-')">
        ...
        <div class=" items-center md:gap-4 flex ">
            ...
            <button class="p-1 rounded-xl md:flex hidden items-center justify-center gap-2" v-if="user?.role">
                <img :src="`/images/avatars/${user?.role}.png`" class="md:h-14 md:w-14 h-10 w-10 p-1"
                    style="border-radius:12px" />
                <div class="flex flex-col items-start">
                    <p class="md:text-sm text-xs font-semibold text-gray-100">{{ user?.role == 'admin' ? 'Manager' :
                        user?.role ==
                        'designer' ?
                        'Designer' : user?.role == 'user' ? 'Personal' : ''
                        }}</p>
                    <p class="md:text-sm text-[9px] font-normal text-gray-300">{{ user?.mobile.slice(-4) }}...{{
                        user?.mobile.slice(0, 3) }}</p>
                </div>
            </button>
        </div>
        <el-drawer v-model="isMenuOpen" direction="rtl" size="70%" @close="isMenuOpen = false" class="menu-drawer">
            ...
            <div class="flex items-center justify-between px-3 mb-6">
                <button
                    ...
                    <img :src="`/images/avatars/${user?.role || 'user'}.png`" class="h-14 w-14 p-1"
                        style="border-radius:12px" />
                    <div class="flex flex-col items-start">
                        <p class="text-sm font-semibold text-gray-500 mb-2">{{
                            user?.role == 'admin' ? 'Manager' : user?.role == 'designer' ? 'Designer' : 'Personal' }}</p>
            ...
            <ul>
                <li v-for="(item, index) in menuOptions" :key="index" class="w-full h-fit px-3"
                    ...
                    <button
                        ...
                        :class="(item.name == poolType ? 'text-primary-b1 md:bg-opacity-50' : 'text-primary-b2 md:bg-opacity-15 bg-opacity-0'), index !== 0 && 'border-t'"
    ...
    <div class="md:hidden">
        ...
        <CanvasRight :type="props.type" :visible="props.visible"
            ...
            :class="!props.visible ? 'md:right-[-400px] md:bottom-0 md:top-0 bottom-[5000px]' : 'md:right-[0] md:bottom-0 md:top-0 top-[-200px] bottom-0'"
...
import { useAuthStore, useMenuStore, useSnapshotStore, useFabricStore, useStudioStore, useMainStore } from '@/store'
