      ...
      <div v-for="(item, index) in illustrationCategoryData" :key="index">
        <el-row class="col-tip mt-5">
          ...
          <el-col :span="7" class="col-name">
            <el-button text @click="showTotal(item.type)">All
              <Icon name="arrow-right" />
            </el-button>
          </el-col>
        </el-row>
        <el-row class="category-box mt-5" v-loading="item.category.length === 0">
          <div class="box-image" v-for="(img, index) in item.category" :key="index"
    ...
    <div class="category-container" ref="totalRef" @scroll="onTotalScroll" v-else>
      <el-row class="col-tip mt-5">
        <el-col :span="7" class="col-name">
          <el-button text @click="hideTotal()">
            <Icon name="arrow-left" />{{ categoryData.name }}
          </el-button>
        </el-col>
      </el-row>
      <el-row class="total-box mt-5" v-loading="categoryData.total.length === 0">
        <div class="box-image" v-for="(img, index) in categoryData.total" :key="index">
          <!-- <el-tooltip placement="top" :content="img.tags" :hide-after="0"> -->
          <img :src="img.previewURL" :alt="img.tags" @click="createImage(img)" lazy loading="lazy"
            :ref="(e: any) => setItemStyle(e, index)" />
          <!-- </el-tooltip> -->
        </div>
      </el-row>
    </div>
    <el-row class="image-bottom">End~</el-row>
  </div>
</template>
...
const hideLoading = async (item: any, loading: Image) => {
