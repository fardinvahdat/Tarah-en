<template>
  <el-dialog v-model="dialogVisible" width="35%" class="export-dialog" :before-close="closeExport">
    <div class="export-dialog">
      <div class="tabs">
        <div class="tab" :class="{ 'active': tab.key === exportType }" v-for="tab in tabs" :key="tab.key"
          @click="setExportType(tab.key)">{{ tab.label }}</div>
      </div>
      <div class="content">
...
const tabs: TabItem[] = [
  { key: 'image', label: 'دریافت تصاویر' },
  { key: 'svg', label: 'دریافت SVG' },
  { key: 'pdf', label: 'دریافت PDF' },
  // { key: 'psd', label: 'دریافت PSD' },
  { key: 'json', label: 'دریافت JSON' },
]
