...
export function renderRotateIcon(
  ctx: any,
  left: any,
  top: any,
  styleOverride: any,
  ...
  const scale = 1; // Overall scale
  const circleRadius = 12; // Circle radius (half of 24)
  const iconScale = 0.7; // Scale down the icon to create padding
  const centerOffset = -12; // Center the original SVG
  ...
  ctx.fillStyle = "white";
  ctx.fill();
  ctx.strokeStyle = "#22319e";
  ctx.lineWidth = 1;
  ctx.stroke();

  // Save transform to scale just the icon
  ctx.save();
  ctx.scale(iconScale, iconScale);

  // Draw the SVG paths centered with padding
  ctx.beginPath();

  // First path (adjusted by centerOffset)
  ctx.moveTo(22 + centerOffset, 12 + centerOffset);
  ctx.bezierCurveTo(
    22 + centerOffset,
    18 + centerOffset,
    17.6095 + centerOffset,
    22 + centerOffset,
    12.1935 + centerOffset,
    22 + centerOffset
  );
  ctx.bezierCurveTo(
    7.79192 + centerOffset,
    22 + centerOffset,
    4.24063 + centerOffset,
    19.6651 + centerOffset,
    3 + centerOffset,
    16 + centerOffset
  );

  // Second path (adjusted by centerOffset)
  ctx.moveTo(2 + centerOffset, 12 + centerOffset);
  ctx.bezierCurveTo(
    2 + centerOffset,
    6 + centerOffset,
    6.3905 + centerOffset,
    2 + centerOffset,
    11.8065 + centerOffset,
    2 + centerOffset
  );
  ctx.bezierCurveTo(
    16.2081 + centerOffset,
    2 + centerOffset,
    19.7594 + centerOffset,
    4.33489 + centerOffset,
    21 + centerOffset,
    8 + centerOffset
  );

  // Third path (adjusted by centerOffset)
  ctx.moveTo(6.99999 + centerOffset, 17 + centerOffset);
  ctx.lineTo(3.00001 + centerOffset, 16 + centerOffset);
  ctx.lineTo(2 + centerOffset, 20 + centerOffset);

  // Fourth path (adjusted by centerOffset)
  ctx.moveTo(17 + centerOffset, 7 + centerOffset);
  ctx.lineTo(21 + centerOffset, 8 + centerOffset);
  ctx.lineTo(22 + centerOffset, 4 + centerOffset);

  ctx.strokeStyle = "#22319e";
  ctx.lineWidth = 2 / scale / iconScale; // Adjust line width for icon scaling
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.stroke();

  ctx.restore(); // Restore to pre-icon scaling
